...
`--device` | Platforms/devices list for OpenCL based engines. Format: <Platform1 ID>:<Device1>,<Device2>;<Platform2 ID>:<Device1>,<Device2>... For example:  `--device=0:0,1` - first and second devices from first platform (with same context), `--device=0:0;0:1` - first and second devices from first platform (with separate contexts)
