...
nbody_engine_block_vv::nbody_engine_block_vv()
{
}

const char* nbody_engine_block_vv::type_name() const
{
	return "nbody_engine_block_vv";
}

void nbody_engine_block_vv::fcompute( const nbcoord_t& t, const memory* _y, memory* _f, size_t yoff, size_t foff )
{
	Q_UNUSED(t);
	...
	for( size_t n1 = 0; n1 < count; n1 += block )
	{
		...
		for( size_t n2 = 0; n2 < count; n2 += block )
		{
			...
			for( size_t b1 = 0; b1 != block; ++b1 )
			{
				for( size_t b2 = 0; b2 != block; ++b2 )
				{
					static const nbcoord_t a_dissip = 0.01;
					static const nbcoord_t a_vv = 100000;
