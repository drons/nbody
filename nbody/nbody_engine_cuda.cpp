...
void nbody_engine_cuda::synchronize_f_heap(nbody_engine_cuda::smemory* f, int* indites)
{
	size_t	count = f->size() / sizeof(nbcoord_t);
	size_t	points_count = m_data->get_count();
	size_t	stride = points_count;
	size_t	tree_offset = points_count;
	std::vector<nbcoord_t>	total_buff(count, -1e11);

	#pragma omp parallel num_threads(m_device_ids.size())
	{
		size_t	dev_n = static_cast<size_t>(omp_get_thread_num());
		std::vector<nbcoord_t>	current_buff(count, -1e11);
		cudaSetDevice(m_device_ids[dev_n]);
		cudaMemcpy(current_buff.data(), f->data(dev_n), f->size(), cudaMemcpyDeviceToHost);

		size_t	dev_point_count(points_count / m_device_ids.size());
		size_t	dev_off(dev_point_count * dev_n);

		for(size_t tn = 0; tn != dev_point_count; ++tn)
		{
			size_t	n1 = static_cast<size_t>(indites[tn + dev_off + tree_offset]);
			total_buff[n1 + 3 * stride] = current_buff[n1 + 3 * stride];
			total_buff[n1 + 4 * stride] = current_buff[n1 + 4 * stride];
			total_buff[n1 + 5 * stride] = current_buff[n1 + 5 * stride];
		}
		cudaDeviceSynchronize();
	}

	write_buffer(f, total_buff.data());
}
...
void nbody_engine_cuda::print_info() const
{
	...
	for(size_t n = 0; n != m_device_ids.size(); ++n)
	{
		...
		qDebug() << "\t #" << m_device_ids[n] << prop.name << "PCI Bus" << prop.pciBusID << "Id"
				 << prop.pciDeviceID << "Domain" << prop.pciDomainID;
