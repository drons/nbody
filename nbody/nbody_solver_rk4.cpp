...
void nbody_solver_rk4::advise(nbcoord_t dt)
{
	...
	engine()->fcompute(t, y, m_k[0], 0, 0);   // k1 = f( t, y )

	engine()->fmadd(m_tmp, y, m_k[0], 0.5 * dt, 0, 0, 0); //tmp = y + 0.5*k1*dt
	engine()->fcompute(t + 0.5 * dt, m_tmp, m_k[1], 0, 0); // k2 = f( t + 0.5*dt, y + 0.5*k1*dt )

	engine()->fmadd(m_tmp, y, m_k[1], 0.5 * dt, 0, 0, 0); //tmp = y + 0.5*k2*dt
	engine()->fcompute(t + 0.5 * dt, m_tmp, m_k[2], 0, 0); // k3 = f( t + 0.5*dt, y + 0.5*k2*dt )

	engine()->fmadd(m_tmp, y, m_k[2], dt, 0, 0, 0); //tmp = y + k3*dt
	engine()->fcompute(t + dt, m_tmp, m_k[3], 0, 0); // k4 = f( t + dt, y + k3*dt )

	//y += 	dt( k1/6 + k2/3 + k3/3 + k4/8 )
	const nbcoord_t	coeff[] = { dt / 6, dt / 3, dt / 3, dt / 6 };

	engine()->fmaddn_inplace(y, m_k, coeff, 0, 0);
